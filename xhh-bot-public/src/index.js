...
async function processMention(msg, botUserId) {
  const commentId = msg.comment_a_id;
  if (!commentId) return;

  // 去重
  if (await db.isReplied(commentId)) return;
...
async function poll(config) {
  const botUserId = config.cookie.heybox_id || '';
  ...
  try {
    ...
    for (const msg of messages) {
      try { await processMention(msg, botUserId); }
      catch (e) { console.log(`  [错误] 处理消息失败: ${e.message}`); }
    }
  } catch (e) {
...
async function showStats() {
  ...
  if (daily.length > 0) {
    console.log('--- 近7天每日 ---');
    daily.forEach(r => console.log(`  ${r.date} | ${r.total} 次 | ${r.unique_users} 人`));
  }
  const hourly = await db.getHourlyStats(true);
  if (hourly.length > 0) {
    console.log('--- 今日每小时 ---');
    hourly.forEach(r => console.log(`  ${r.hour}:00 | ${r.total} 次 | ${r.unique_users} 人`));
  }
  console.log('========================\n');
}
...
async function main() {
  ...
  console.log('  GitHub: SomeOvO/xhhRobot');
  console.log(`  检测间隔: ${config.bot.checkInterval}s`);
  console.log(`  AI 模型: ${config.ai.model}`);
  console.log(`  数据库: ${config.database?.type || 'sqlite'}`);
  console.log('========================================\n');
